...
import { LayoutDashboard, BookOpen, CheckCircle, FolderKanban, BarChart3, Settings, Users, ChevronLeft, User, BarChart3 as SkillIcon } from 'lucide-react';
    ...
    maxWidth: collapsed ? '64px' : '200px'
      ...
      <div className="flex items-center border-b border-sidebar-border h-16 px-4">
        <div className="flex items-center">
          ...
          <div className={`ml-3 text-sidebar-foreground font-semibold text-base whitespace-nowrap transition-all duration-300 overflow-hidden ${collapsed ? 'opacity-0 w-0' : 'opacity-100 w-auto'}`}>
            RealThings
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 py-4">
        <nav className="space-y-1 px-3">
          ...
          const menuButton = <NavLink to={item.url} end={item.url === '/'} className={`
                  ...
                  ${active ? 'text-sidebar-primary bg-sidebar-accent border-r-2 border-sidebar-primary' : 'text-sidebar-foreground hover:text-sidebar-primary hover:bg-sidebar-accent/50'}
                `}>
      ...
      <div className="border-t border-sidebar-border p-3 space-y-1">
        ...
        <div>
          ...
          const profileButton = <button onClick={handleLogout} className="flex items-center h-10 w-full rounded-lg transition-colors font-medium text-sidebar-foreground/70 hover:text-sidebar-primary hover:bg-sidebar-accent/50">
                <div className="w-10 h-10 flex items-center justify-center flex-shrink-0">
